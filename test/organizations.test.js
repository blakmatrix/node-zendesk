...
describe('Zendesk Client Organizations', () => {
  ...
  it('should successfully create a new organization with the expected name', async () => {
    const { nockDone, context } = await nockBack('organizations_test_create.json');
  ...
  it('should include the new organization in the list of all organizations', async () => {
    const { nockDone, context } = await nockBack('organizations_test_list_all.json');
  ...
  it('should retrieve the details of the created organization using its ID', async () => {
    const { nockDone, context } = await nockBack('organizations_test_show_single.json');
  ...
  it('should successfully delete the created organization', async () => {
    const { nockDone, context } = await nockBack('organizations_test_delete_organization.json');
  ...
  it('should throw an error when trying to delete an already deleted or non-existent organization', async () => {
    const { nockDone, context } = await nockBack('organizations_test_delete_not_found_organization.json');
    await expect(() =>
      client.organizations.delete(69420 * 69420),
