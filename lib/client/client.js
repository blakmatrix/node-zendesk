...
Client.prototype.requestAll = function (method, uri) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if ( typeof callbackOpt === 'function' ) {
      ...
      completeCb = callbackOpt.complete || function (statusList, bodyList, responseList, resultList) { resolve(bodyList); };
    }
...
Client.prototype.requestUpload = function (uri, file, isReadStream, callback) {
  var self     = this,
      out,
      auth     = this.options.get('password') ? ':' + this.options.get('password') : '/token:' + this.options.get('token'),
      encoded  = new Buffer(this.options.get('username') + auth).toString('base64'),
  ...
  return new Promise(function(resolve, reject) {
    const stdCallback = function(e, response, result) {
    ...
    if (isReadStream) {
      ...
      file.pipe(out); // pipe the readstream!
    } else if(binary){
