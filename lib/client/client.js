...
Client.prototype.request = function(method, uri) {
    var options,
        ...
        encoded = Buffer.from(this.options.get("username") + auth).toString(
            "base64"
        ),
    ...
    if (self)
        if (self.options)
            if (self.options.formData) delete self.options.formData;
...
Client.prototype.requestUploadForm = function(uri, buffer, callback) {
    var self = this,
        out,
        auth = this.options.get("password") ?
        ":" + this.options.get("password") :
    ...
    // form.append("user[photo][uploaded_data]", buffer)
    ...
    self.options.headers = {};
    // self.options.headers = { 'Content-Type': `multipart/form-data; boundary=${buffer.byteLength}` };
    if (useOAuth) {
        self.options.headers.Authorization = "Bearer " + token;
    } else {
        self.options.headers.Authorization = "Basic " + encoded;
