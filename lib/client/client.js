...
Client.prototype.requestAll = function (method, uri) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (typeof callbackOpt === 'function') {
      ...
      pageLimit = callbackOpt.pageLimit || pageLimit;
    ...
    function processPage(status, body, response, result) {
      if (completeCb) { // only accumulate pages if a completeCb is provided
        ...
        bodyList.push(body.tickets || body);
        responseList.push(response);
        resultList.push(result);
      }
      // result.links.next is used for cursor pagination from search.exportAll
      // result.after_url is used in ticket incremental export
      nextPage = result ? result.next_page || result.after_url || (result.meta && result.meta.has_more && result.links && result.links.next) : null;
    ...
    return __request.apply(self, args.concat(function (error, status, body, response, result) {
      ...
      async.whilst(
        function (cb) {
          const isEndOfIncremental = isIncremental && count < 1000;
          // If you request a page past the limit (page=11 at 100 results per page), a 422 Insufficient Resource Error is returned.
          // see https://developer.zendesk.com/api-reference/ticketing/ticket-management/search/
          const isEndOfSearchExport = uri[0] === 'search' && uri[1] !== 'export' && nextPage.includes('page=11');
          const isPageLimit = bodyList.length > 0 && bodyList.length === pageLimit;
          if (nextPage === null || isEndOfIncremental || isEndOfSearchExport || isPageLimit) {
            nextPage = '';
          }
          cb(null, nextPage);
        },
